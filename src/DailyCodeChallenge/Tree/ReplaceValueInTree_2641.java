...
import _16_stack_queue.Algo.CircularQueue;

import java.util.ArrayDeque;
import java.util.ArrayList;
import java.util.List;

class ReplaceValueInTree_2641 {
    ...
    private void DFS(TreeNode root, int level, List<Integer> levelSum) {
        ...
        int currSum = levelSum.size() > (level + 1) ? levelSum.get(level + 1) : Integer.MIN_VALUE;

        if (currSum > left + right) {
            if (root.left != null) root.left.val = currSum - left - right;
            if (root.right != null) root.right.val = currSum - left - right;
        } else if (currSum == left + right) {
