...
class MinimumWeight_2203_4 {
    public long minimumWeight(int n, int[][] edges, int src1, int src2, int dest) {
        HashMap<Long, List<long[]>> adj = new HashMap<>();
        HashMap<Long, List<long[]>> reAdj = new HashMap<>();
        for (int[] edge : edges) {
            long u = edge[0], v = edge[1], w = edge[2];
    ...
    private long[] dijkstra(int src, HashMap<Long, List<long[]>> adj, int nv) {
    ...
    public static void main(String[] args) {
        ...
        long ans = new MinimumWeight_2203_4().minimumWeight(n, edges, 0, 1, 5);
