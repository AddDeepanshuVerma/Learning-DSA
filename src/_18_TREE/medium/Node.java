...
// copied and pasted from Google, have already done bruteforce for the same q. (1993. Operations on Tree)s
class Node {
...
class LockingTree {
    ...
    public boolean lock(int num, int user) {
        if (nodes[num].lockedBy != -1) return false;
        nodes[num].lockedBy = user;
        return true;
    }

    public boolean unlock(int num, int user) {
        if (nodes[num].lockedBy != user) return false;
    ...
    private boolean anyLockedDescendant(int i) {
        return nodes[i].lockedBy != -1 || nodes[i].children.stream().anyMatch(child -> anyLockedDescendant(child));
    }
