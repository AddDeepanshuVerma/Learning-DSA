...
class SubtreeInversionSum_3544_1Recursion {
    ...
    private long dfs(int u, int lca, int sign, int parent, int k, HashMap<Integer, List<Integer>> adj, int[] nums) {
        ...
        if (lca >= k) {
            long invert = (long) sign * nums[u] * -1;
            for (Integer v : adj.getOrDefault(u, Collections.emptyList())) {
                if (v == parent) continue;
                invert += dfs(v, 1, sign * -1, u, k, adj, nums);
            }
    ...
    public static void main(String[] args) {
        ...
        int k = 2;
        var obj = new SubtreeInversionSum_3544_1Recursion();
