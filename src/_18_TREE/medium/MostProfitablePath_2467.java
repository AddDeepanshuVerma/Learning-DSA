...
class MostProfitablePath_2467 {
    public static void main(String[] args) {
        MostProfitablePath_2467 obj = new MostProfitablePath_2467();
    ...
    public int mostProfitablePath(int[][] edges, int bob, int[] amount) {
        ...
        int value = amount[0];
        if (bob == 0) value = amount[0] >> 1;  // if alice and bob are on same node at start
        Pair pair = new Pair(0, value);

        queue.addLast(pair); // adding alice position
        visited.add(0); // adding alice position as visited
//        amount[stack.pop()] = 0;
