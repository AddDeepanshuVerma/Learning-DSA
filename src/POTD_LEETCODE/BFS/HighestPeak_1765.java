...
class HighestPeak_1765 {
    @Description("BFS Using Priority queue")
    ...
    @Description("BFS Using deQueue + slight diff")
    public int[][] highestPeak_opt2(int[][] isWater) {
        int m = isWater.length, n = isWater[0].length;
        ArrayDeque<int[]> dQue = new ArrayDeque<>();
        boolean[][] seen = new boolean[m][n];
        int[][] dirs = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}}; // right, left, up, down
        ...
        while (!dQue.isEmpty()) {
            ...
            while (size-- > 0) {
                ...
                for (int[] dir : dirs) {
                    ...
                    dQue.offer(new int[]{value + 1, nRow, nCol});
                    isWater[nRow][nCol] = value + 1;
                    seen[nRow][nCol] = true;
                }
            }
